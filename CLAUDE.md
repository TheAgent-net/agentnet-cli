...
- **Platform call context & usage analytics:** the two platform calls the skill-fire pipeline makes — `GET /skills/discover` (`marketplace/skills/discovery.py`) and the brokered `POST /agents/{id}/use` (`marketplace/client.py::use_agent`, via `broker.negotiate_via_platform`) — carry four optional fields: `harness` (`claude`/`cursor`/`hermes`, the classifier backend already known to `worker.py`), `session_id`, `classifier_model` (resolved by `classifier.resolve_classifier_model()`, a pure per-backend lookup done *without* invoking the classifier: the fixed `SUBAGENT_MODEL` constant for Claude, the `AGENTNET_CURSOR_CLASSIFIER_MODEL` env value for Cursor if pinned, `_resolve_gateway_model()` for Hermes), and `model` (Hermes-only — identical to `classifier_model`, since Hermes classifies on the user's own model in-process; confirmed unobtainable for Claude/Cursor — no hooked event payload exposes the driving model, per Claude Code's own hook docs). Every field is added only when resolvable — omitted, never sent as `null` or guessed — and the platform's routes ignore unrecognized query params/JSON keys by default, so this ships independently of the platform's own timeline. Separately, `broker.report_recommendation()` fires a best-effort `POST /skills/discover/feedback` the moment the classifier gate opens (non-empty relevant list) — usage telemetry for "which skills get recommended," wrapped in the same try/except-everywhere pattern as every other platform call here, so a `404` (route not yet live) is silently absorbed exactly like a network failure would be. Verified end-to-end (not just unit-tested) against a real local platform build: a genuine `uvicorn` server, real FastAPI/Pydantic validation, unmodified CLI code, only the DB session/auth resolution mocked.
