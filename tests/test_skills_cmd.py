...
@patch("agentnet_cli.skills.client.SkillsClient")
def test_search_happy_path(mock_cls, fake_home):
    payload = {
        "skills": [{"id": "org/repo/skill", "name": "code-review", "installs": 100}],
        "count": 1,
    }
...
@patch("agentnet_cli.skills.client.SkillsClient")
def test_search_with_limit(mock_cls, fake_home):
    mock_cls.return_value = _mock_client(search={"skills": [], "count": 0})
    result = runner.invoke(app, ["skills", "search", "test", "--limit", "5"])
    assert result.exit_code == 0
    mock_cls.return_value.search.assert_called_once_with(query="test", limit=5)
...
@patch("agentnet_cli.skills.skillsmp.SkillsMPClient")
def test_search_skillsmp_source(mock_cls, fake_home):
    payload = {
        "data": [{"id": "s1", "name": "code-review", "author": "alice"}],
        "pagination": {"page": 1, "total": 1},
    }
    mock_cls.return_value = _mock_client(search=payload)
    result = runner.invoke(app, ["skills", "search", "code review", "--source", "skillsmp"])
    assert result.exit_code == 0
    data = json.loads(result.stdout)
...
@patch("agentnet_cli.skills.skillsmp.SkillsMPClient")
def test_search_skillsmp_with_options(mock_cls, fake_home):
