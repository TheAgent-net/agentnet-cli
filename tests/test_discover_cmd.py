...
def test_discover_happy_path(mock_gc, fake_home):
    mock_gc.return_value = _mock_client(
        search={"query": "code review", "type": "all", "results": [{"name": "CodeBot", "id": "cb-1"}]}
    )
    result = runner.invoke(app, ["discover", "code review"])
    assert result.exit_code == 0
    data = json.loads(result.stdout)
    assert data["results"][0]["name"] == "CodeBot"


@patch("agentnet_cli.cli.marketplace.discover.get_client")
def test_discover_with_limit(mock_gc, fake_home):
    mock_gc.return_value = _mock_client(search={"query": "weather", "type": "all", "results": []})
    result = runner.invoke(app, ["discover", "weather", "--limit", "3"])
    assert result.exit_code == 0
    mock_gc.return_value.search.assert_called_once_with(query="weather", kind="all", limit=3)
...
def test_discover_platform_error(mock_gc, fake_home):
    ...
    mock_gc.return_value = _mock_client()
    mock_gc.return_value.search.side_effect = PlatformError("Platform server error")
